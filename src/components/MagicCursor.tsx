  ...
  return (
    <>
      ...
      {trail.map((point, idx) => (
        <div
          key={point.id}
          className="pointer-events-none fixed w-2 h-2 rounded-full bg-gradient-to-r from-blue-500 to-gray-500 z-40 animate-pulse"
          style={{
            left: `${point.x}px`,
            top: `${point.y}px`,
      ...
      <div
        className={`pointer-events-none fixed rounded-full z-50 transition-all duration-200 ${
          isHovering 
            ? 'w-4 h-4 bg-gradient-to-r from-blue-500 via-gray-500 to-gray-500' 
      ...
      <div
        className={`pointer-events-none fixed rounded-full border-2 z-50 transition-all duration-300 ${
          isHovering 
            ? 'w-14 h-14 border-blue-400 animate-spin-slow' 
            : 'w-10 h-10 border-blue-500/50'
        }`}
        style={{
          left: `${mousePos.x}px`,
          top: `${mousePos.y}px`,
      ...
      <div
        className={`pointer-events-none fixed rounded-full border z-40 transition-all duration-300 ${
          isHovering ? 'w-8 h-8 border-gray-400' : 'w-6 h-6 border-blue-400/30'
        }`}
        style={{
          left: `${mousePos.x}px`,
          top: `${mousePos.y}px`,
        ...
        <>
          {[0, 90, 180, 270].map((angle, idx) => {
            ...
            return (
              <div
                key={idx}
                className="pointer-events-none fixed w-1.5 h-1.5 rounded-full bg-gray-400 z-40"
                style={{
                  left: `${x}px`,
                  top: `${y}px`,
                  transform: 'translate(-50%, -50%)',
